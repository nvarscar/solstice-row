...
- **Teams persistence** — team registrations are stored in **SQLite** (`teams.db`) in the `solstice_data` Docker volume at `/data/teams.db`. The DB is created automatically on first access via `lib/db.ts` (`getDb()`). Rebuilding the image never destroys registration data. All team reads/writes go through `lib/db.ts`. Per-team updates use `PATCH /api/content/teams/:id`; deletions use `DELETE /api/content/teams/:id`.
